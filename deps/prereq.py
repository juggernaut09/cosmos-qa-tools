...
if is_tool('go'):
    ...
    print("------ Update bashrc ---------------")
    subprocess.run(['export', 'GOPATH={}/go'.format(os.environ['HOME'])])
    subprocess.run(['export', 'GOROOT=/usr/local/go'])
    subprocess.run(['export', 'GOBIN={}/bin'.format(os.environ['GOPATH'])])
    subprocess.run(['export', 'PATH={}:/usr/local/go/bin:{}'.format(os.environ['PATH'], os.environ['GOBIN'])])
    ...
    bash_object.close()
    subprocess.run(['sudo', '.', '{}/.bashrc'.format(os.environ['HOME'])])
